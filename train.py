...
# Memory & Training Hyperparameters
BATCH_SIZE = 8
GRAD_ACCUM_STEPS = 4 
MAX_STEPS = 10000
VAL_INTERVAL = 250
...
# ========================================== #
def configure_optimizers(model: nn.Module, lr: float, wd: float) -> torch.optim.Optimizer:
    """Decouples regularized weight groups from weight decay."""
    decay_params = []
    no_decay_params = []
    blacklist_classes = (nn.Embedding, nn.LayerNorm)
    if hasattr(nn, "RMSNorm"):
        blacklist_classes += (nn.RMSNorm,)
    ...
    has_fused = "fused" in torch.optim.AdamW.__init__.__code__.co_varnames
    use_fused = DEVICE == "cuda" and has_fused
    print(f"[Optimizer] Decay: {len(decay_params)} | No-Decay: {len(no_decay_params)} | Fused: {use_fused}")
    return torch.optim.AdamW(opt_groups, lr=lr, betas=(0.9, 0.95), eps=1e-8, fused=use_fused)


# ========================================== #
# 3. DYNAMIC COSINE TRAINING LR SCHEDULE     #
# ========================================== #
def get_adaptive_lr(step: int, max_steps: int, warmup: int, base_lr: float, min_lr: float) -> float:
...
# ========================================== #
@torch.no_grad()
def evaluate_loss(model: nn.Module, loader: DataLoader, device: str, steps: int = 50) -> float:
...
def main():
    ...
    print("[Pipeline] Configuring token dataset streams...")
    train_ds = HyperionDataset(TRAIN_TXT, tokenizer, max_seq_len=v3_config.max_seq_len)
    val_ds = HyperionDataset(VAL_TXT, tokenizer, max_seq_len=v3_config.max_seq_len) if os.path.exists(VAL_TXT) else None

    train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True, drop_last=True, pin_memory=True)
    val_loader = DataLoader(val_ds, batch_size=BATCH_SIZE, shuffle=False, drop_last=True, pin_memory=True) if val_ds else None

    model = HyperionV3(config=v3_config).to(DEVICE)
    
    # Configure optimizer BEFORE model compilation
    optimizer = configure_optimizers(model, LEARNING_RATE, WEIGHT_DECAY)

    if hasattr(torch, "compile") and DEVICE == "cuda":
        print("[Pipeline] Compiling model with torch.compile()...")
    ...
    while step < MAX_STEPS:
        for x_b, y_b in train_loader:
            ...
            # Optimization block resolved on accumulation boundary limits
            if micro_step_count == GRAD_ACCUM_STEPS:
                ...
                # Learning rate scheduling steps
                current_lr = get_adaptive_lr(step, MAX_STEPS, WARMUP_STEPS, LEARNING_RATE, MIN_LR)
                for pg in optimizer.param_groups:
                    pg["lr"] = current_lr

                # Evaluation Log Processing Phase
                if step % VAL_INTERVAL == 0 and step > 0:
                    train_loss_print = accumulated_loss / GRAD_ACCUM_STEPS
                    if val_loader:
                        v_loss = evaluate_loss(model, val_loader, DEVICE)
                        print(f"Step: {step:05d} | Train Loss: {train_loss_print:.4f} | Val Loss: {v_loss:.4f} | GradNorm: {total_norm:.2f} | LR: {current_lr:.2e}")
                        ...
                        print(f"Step: {step:05d} | Train Loss: {train_loss_print:.4f} | GradNorm: {total_norm:.2f} | LR: {current_lr:.2e}")

                if step % SAVE_INTERVAL == 0 and step > 0:
                    p_path = os.path.join(CHECKPOINT_DIR, f"hyperion_v3_{step}.pt")
                    torch.save({"model_state_dict": model.state_dict()}, p_path)

                # CRITICAL RESETS: Increment step and clear counters!
                step += 1
                accumulated_loss = 0.0
                micro_step_count = 0
